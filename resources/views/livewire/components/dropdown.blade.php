<div class="odecci-dropdown"><input name="department" class="dropdown" placeholder="Department"  autocomplete="off" readonly>
    <div class="odecci-data-container">
        <input class="search-selection"  autocomplete="off" placeholder="Search {{ $title }}"> 
        <div class="odecci-selections">
            @foreach ($selections as $selection )
            <a class="odecci-data" >{{ $selection }}</a>
            @endforeach
            <a class="odecci-no-data">No Data Found</a>
            <a class="odecci-new-data"><svg width="20" height="20" viewBox="0 0 20 20" fill="none"
    ...
    <label class="input-label " for="department"> {{ $title }} </label>
</div>
